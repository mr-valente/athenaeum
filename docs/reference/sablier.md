...
Each route in `deploy/caddy/routes.caddy` names a **session tier**, a snippet in `apps.caddy` that passes a `session_duration` to Sablier on every request: `light` (72h), `middle` (12h), `heavy` (4h) and `superheavy` (1h). The tier is chosen by an app's measured memory when awake, so a light app is rarely found asleep while a heavy one gives its memory back between uses; the [app skill](../../skills/athenaeum-app/SKILL.md) has the table and the placement rule. Each duration can be overridden through `SABLIER_SESSION_<TIER>` in the edge container's environment, which the smoke test uses to shorten `light` to 15 seconds.

Sessions are **sliding**: every request routed through an app renews its session, and the in-memory store checks for expiry about once a second (Sablier 1.18.0 accepts `sessions.expiration-interval` but does not apply it to that store). A tier is not a hard lifetime from first startup. An open page that keeps polling can keep an app awake indefinitely. Background jobs and idle WebSockets alone do not renew the session.

`sablier.yaml`'s `default-duration` is 1h, the shortest tier. It reaches only an app Sablier finds already running without a session — after a deployment or its own restart, or through the self-heal described next — so nothing heavy is kept awake by accident; `athenaeumctl verify` then renews every app at its tier.
...
`deploy/sablier/themes/athenaeum.html` follows the After hours contract: ink canvas, warm ivory text, mint accents, amber keyboard focus, monospace fallback, flat borders and the shared tesseract mark. The status light breathes slowly, the one looping animation the style contract allows, and holds still under reduced motion; there is no JavaScript or external asset service. The layout works on narrow screens; refresh and manual retry preserve the requested path and query.

Compose mounts `design/` as the theme's `assets/` directory. Sablier embeds the SVG into the response, so the loading page never needs to request assets from a sleeping app. The template uses `.DisplayName` and `.RefreshFrequency`; session lengths and other infrastructure details are hidden. Config/theme files are read-only mounts from the checkout. `athenaeumctl docker deploy` and `docker pull --deploy` recreate Sablier on every run, after applying the stack, so policy, theme and asset changes take effect on deployment. Compose alone would not: it recreates a service when its definition changes, not when a mounted file does, and Git replaces `sablier.yaml` beneath the container's single-file mount. The recreation discards Sablier's in-memory timers; running apps are adopted at the default session and `verify` renews each at its tier. The same step by hand, for a checkout change made without deploying:
