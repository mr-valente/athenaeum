...
First [prepare or retrieve the age recovery identity](5-backup-and-recovery.md#prepare-the-recovery-identity) on your workstation. Keep the private identity outside the VM except during a restore/drill. Have its public recipient and the values from Guide 1 ready.
...
| Backup bucket cap | Your allocation in bytes; see [Guide 5's calculation](5-backup-and-recovery.md#choose-the-bucket-cap) |
...
Complete [Guide 5's recovery drill and offsite copy](5-backup-and-recovery.md#recovery-drill-and-offsite-copy), including its final backup-timer enable command. Return here once the archive has restored successfully and the workstation copy has passed its checksum check.
